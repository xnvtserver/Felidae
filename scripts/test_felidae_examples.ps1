...
$directTests = @(
    ...
    @{ Name = "unified directional fact comparison"; Args = @("examples\expert_system_acceptance.fx"); Expect = @('state: "animal-interpreted"', 'judgment: "target-owned"', 'relationshipCount: 1', 'matchedAncestor: "Mammal"', 'ancestorDistance: 1', 'previousState: "animal-interpreted"', 'pipelined: {state: "animal-interpreted"', 'resolved_fields: {state: "animal-interpreted"', 'state: "unresolved"', 'reason: "required dependency is not satisfied"', 'state: "exact-member"', '__type: "Comparison"') },
    ...
    @{ Name = "in-memory expert intelligence system"; Args = @("examples\expert_intelligence_system.fx"); Expect = @('state: "approved"', 'rule: "high_credit_with_verified_income"', 'state: "unresolved"', 'reason: "required dependency is not satisfied"', 'state: "declined"', 'rule: "low_credit"') },
    @{ Name = "dynamic fact references"; Args = @("examples\dynamic_fact_references.fx"); Expect = @('__type: "ReferenceResult"', '__type: "Velocity"', 'value: 5', '__type: "WaveProjection"', 'value: 20', 'velocity_at_five: [{__type: "ReferenceResult", result: {__type: "Velocity", value: 2}') },
