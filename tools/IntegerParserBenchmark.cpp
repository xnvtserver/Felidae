...
int main(int argc, char **argv) {
  const bool nativeExpression =
      argc == 3 && std::string_view(argv[1]) == "--native-expression";
  ...
  if (nativeExpression) {
    ...
    auto largestSymbol = entry;
    for (const auto symbol : ir.symbols)
      largestSymbol = std::max(largestSymbol, symbol);
    module.sentencePieceModelIdentity = "sha256:benchmark";
    ...
    (void)vm.executeMain(Felidae::verifyIrModule(std::move(module)), runtime,
                         Felidae::VmNil{});
  ...
  if (argc != 2 && !dumpIds) {
    std::cerr
        << "usage: felidae_integer_parser_benchmark [--dump-ids] <source.fx>\n"
        << "       felidae_integer_parser_benchmark --native-expression "
           "<expression>\n";
    return 2;
  }
  std::ifstream input(argv[sourceArgument], std::ios::binary);
  if (!input)
    throw std::runtime_error("cannot open benchmark source");
