...
TokenizerCorpus loadCorpus(const std::string& path) {
    ...
    while (std::getline(input, line)) {
        ++lineNumber;
        for (const char rawByte : line) {
            corpus.hash ^= static_cast<unsigned char>(rawByte);
            corpus.hash *= 1099511628211ull;
        }
        corpus.hash ^= static_cast<unsigned char>('\n');
        corpus.hash *= 1099511628211ull;
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        ids += "inline constexpr std::uint64_t kFelidaeTokenizerDatasetHash = " +
               std::to_string(dataset.hash) + "ull;\n";
        ...
        ids += "inline constexpr std::uint64_t kFelidaeSentencePieceModelHash = " +
               std::to_string(fnv1a(serialized)) + "ull;\n";
