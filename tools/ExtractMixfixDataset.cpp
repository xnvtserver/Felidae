...
std::size_t byteOffset(const std::string& source, int line, int column) {
    if (line < 1 || column < 1) throw std::runtime_error("mixfix AST span is invalid");
    std::size_t offset = 0;
    for (int current = 1; current < line; ++current) {
        const auto newline = source.find('\n', offset);
        if (newline == std::string::npos) throw std::runtime_error("mixfix AST span exceeds source lines");
        offset = newline + 1;
    }
    offset += static_cast<std::size_t>(column - 1);
    if (offset > source.size()) throw std::runtime_error("mixfix AST span exceeds source columns");
    return offset;
}

std::vector<SentencePieceId> spanIds(const IntegerTokenList& tokens, const SourceSpan& span) {
    const auto first = byteOffset(tokens.source(), span.startLine, span.startColumn);
...
MixfixVocabularyId token(const MixfixContext& context, MixfixIrTokenKind kind, IrWord value) {
    const auto found = std::find_if(context.outputVocabulary.begin(), context.outputVocabulary.end(),
        [&](const MixfixIrToken& item) { return item.kind == kind && item.value == value; });
    if (found == context.outputVocabulary.end()) {
        throw std::runtime_error("mixfix IR cannot be represented by the fixed vocabulary (kind=" +
                                 std::to_string(static_cast<unsigned>(kind)) + ", value=" + std::to_string(value) + ")");
    }
    return static_cast<MixfixVocabularyId>(std::distance(context.outputVocabulary.begin(), found));
}
...
std::vector<MixfixVocabularyId> encodeTeacher(const FelidaeIr& ir) {
    ...
    const auto symbol = [&](IrWord value) { result.push_back(token(context, MixfixIrTokenKind::SymbolReference, value)); };
    for (std::size_t pc = 0; pc < ir.words.size();) {
        ...
        const auto require = [&](std::size_t width) {
            if (width > ir.words.size() - pc) throw std::runtime_error("deterministic mixfix teacher IR is truncated");
        };
        ...
        result.push_back(token(context, MixfixIrTokenKind::Opcode, static_cast<IrWord>(opcode)));
        switch (opcode) {
        case IrOpcode::End: break;
        case IrOpcode::Jump: require(2); raw(ir.words[pc + 1]); pc += 2; break;
        case IrOpcode::LoadConst: require(3); reg(ir.words[pc + 1]); constant(ir.words[pc + 2]); pc += 3; break;
        case IrOpcode::LoadSymbol: case IrOpcode::StoreSymbol: case IrOpcode::CallNative: case IrOpcode::MakeFact:
            require(3); reg(ir.words[pc + 1]); symbol(ir.words[pc + 2]); pc += 3; break;
        case IrOpcode::Move: case IrOpcode::JumpIfFalse: case IrOpcode::Return:
        ...
        case IrOpcode::CallNamed: case IrOpcode::MakeMap: {
            require(4); reg(ir.words[pc + 1]);
            if (opcode == IrOpcode::CallNamed) symbol(ir.words[pc + 2]); else raw(ir.words[pc + 2]);
            const auto count = ir.words[pc + 3]; raw(count); require(4 + count * 2);
            for (std::size_t index = 0; index < count; ++index) { symbol(ir.words[pc + 4 + index * 2]); raw(ir.words[pc + 5 + index * 2]); }
            pc += 4 + count * 2; break;
        }
        case IrOpcode::Count: throw std::runtime_error("deterministic mixfix teacher has invalid opcode");
        }
    }
...
void visitExpr(const std::shared_ptr<Expr>& expression, std::vector<std::shared_ptr<OperatorExpression>>& output) {
    if (!expression) return;
    if (const auto operation = std::dynamic_pointer_cast<OperatorExpression>(expression)) {
        if (operation->coreOperator == CoreOperator::Unknown) output.push_back(operation);
        for (std::size_t index = 0; index < operation->captureCount(); ++index) visitExpr(operation->capture(index), output);
    } else if (const auto array = std::dynamic_pointer_cast<ArrayExpr>(expression)) {
        for (const auto& item : array->items) visitExpr(item, output);
    } else if (const auto map = std::dynamic_pointer_cast<MapExpr>(expression)) {
        for (const auto& entry : map->entries) visitExpr(entry.value, output);
        ...
        for (const auto& argument : term->args) visitExpr(argument.value, output);
    }
}

void visitCall(const Call& call, std::vector<std::shared_ptr<OperatorExpression>>& output) {
    for (const auto& argument : call.args) visitExpr(argument.value, output);
}

void visitGoal(const std::shared_ptr<Goal>& goal, std::vector<std::shared_ptr<OperatorExpression>>& output) {
    if (const auto call = std::dynamic_pointer_cast<CallGoal>(goal)) visitCall(call->call, output);
    else if (const auto binary = std::dynamic_pointer_cast<BinaryGoal>(goal)) { visitExpr(binary->left, output); visitExpr(binary->right, output); }
    else if (const auto assign = std::dynamic_pointer_cast<AssignGoal>(goal)) visitExpr(assign->expr, output);
    else if (const auto assign = std::dynamic_pointer_cast<MultiAssignGoal>(goal)) visitExpr(assign->expr, output);
    else if (const auto where = std::dynamic_pointer_cast<WhereGoal>(goal)) visitGoal(where->condition, output);
    else if (const auto conditional = std::dynamic_pointer_cast<IfGoal>(goal)) {
        visitGoal(conditional->condition, output); for (const auto& item : conditional->thenBranch) visitGoal(item, output); for (const auto& item : conditional->elseBranch) visitGoal(item, output);
    } else if (const auto returned = std::dynamic_pointer_cast<ReturnGoal>(goal)) {
        for (const auto& field : returned->fields) visitExpr(field.value, output);
    } else if (const auto group = std::dynamic_pointer_cast<GroupGoal>(goal)) {
        for (const auto& item : group->goals) visitGoal(item, output);
    } else if (const auto alternatives = std::dynamic_pointer_cast<OrGoal>(goal)) {
        for (const auto& branch : alternatives->branches) for (const auto& item : branch) visitGoal(item, output);
    }
}

bool isInvalidCase(const std::filesystem::path& path) {
    return std::any_of(path.begin(), path.end(), [](const auto& component) { return component == "invalid"; });
}

std::vector<std::filesystem::path> sources(const std::vector<std::filesystem::path>& roots, bool invalidCases) {
    std::vector<std::filesystem::path> result;
    for (const auto& root : roots) {
        if (std::filesystem::is_regular_file(root) && root.extension() == ".fx" && isInvalidCase(root) == invalidCases) result.push_back(root);
        else if (std::filesystem::is_directory(root)) {
            for (const auto& entry : std::filesystem::recursive_directory_iterator(root)) {
                if (entry.is_regular_file() && entry.path().extension() == ".fx" &&
                    isInvalidCase(entry.path()) == invalidCases) result.push_back(entry.path());
            }
        } else throw std::runtime_error("mixfix dataset input is not a .fx file or directory: " + root.string());
    }
...
std::vector<std::pair<std::string, std::string>> generatedValidSources() {
    constexpr std::string_view transform = R"(@overload(operator: transformWith, pattern: "{value} transform {model} with {count}", type: mixfix, captures: {value: number, model: number, count: number}, result: number, precedence: relationship, associativity: none, cardinality: one, effects: pure, visibility: private)
transformValue() =>
    return value + model * count

main() =>
    return )";
    constexpr std::string_view choose = R"(@overload(operator: chooseOtherwise, pattern: "choose {left} otherwise {right}", type: mixfix, captures: {left: number, right: number}, result: number, precedence: relationship, associativity: right, cardinality: one, effects: pure, visibility: private)
chooseLarger() =>
    if left > right then
        return left
    else
...
main() =>
    return )";
    constexpr std::string_view plan = R"(@mixfix(pattern: "plan {value: number} using {enabled: bool}")
...
main() =>
    ...
    for (std::size_t seed = 1; seed <= 24; ++seed) {
        ...
        result.emplace_back("generated-transform-" + std::to_string(seed),
                            std::string(transform) + left + " transform " + model + " with " + count + "\n");
    }
    for (std::size_t seed = 1; seed <= 24; ++seed) {
        const auto left = std::to_string(seed);
        ...
        result.emplace_back("generated-choose-" + std::to_string(seed),
                            std::string(choose) + "choose " + left + " otherwise choose " + middle +
                                " otherwise " + right + "\n");
    }
    for (std::size_t seed = 1; seed <= 24; ++seed) {
...
void writeJsonl(const std::filesystem::path& output, const std::vector<nlohmann::json>& records) {
    if (output.empty() || output.extension() != ".jsonl") throw std::runtime_error("dataset output must be .jsonl");
    if (records.empty()) throw std::runtime_error("dataset has no records");
    if (!output.parent_path().empty()) std::filesystem::create_directories(output.parent_path());
    const auto temporary = output.string() + ".tmp";
    std::ofstream stream(temporary, std::ios::trunc);
    if (!stream) throw std::runtime_error("cannot write dataset");
    for (const auto& record : records) stream << record.dump() << '\n';
    stream.close();
    if (!stream) throw std::runtime_error("cannot finish dataset");
...
int main(int argc, char** argv) {
    try {
        if (argc < 3) throw std::runtime_error("usage: felidae_extract_mixfix_dataset output.jsonl examples v2_examples [--rejections invalid.jsonl]");
        const std::filesystem::path output(argv[1]);
        if (output.extension() != ".jsonl") throw std::runtime_error("mixfix dataset output must be .jsonl");
        std::vector<std::filesystem::path> roots;
        std::optional<std::filesystem::path> rejectionOutput;
        for (int index = 2; index < argc; ++index) {
            if (std::string_view(argv[index]) == "--rejections") {
                if (rejectionOutput || ++index >= argc) throw std::runtime_error("--rejections requires one output.jsonl path");
        ...
        for (const auto& path : sources(roots, false)) teachers.emplace_back(path.string(), readSourceFile(path));
        auto generated = generatedValidSources();
        teachers.insert(teachers.end(), std::make_move_iterator(generated.begin()), std::make_move_iterator(generated.end()));
        for (const auto& [label, source] : teachers) {
            ...
            try {
                ...
                ++rejectedSources;
                std::cerr << "skipping non-teacher source " << label << ": " << error.what() << '\n';
                continue;
            }
            const IntegerTokenList tokens(felidaeSentencePieceModel(), source);
            std::vector<std::shared_ptr<OperatorExpression>> operations;
            for (const auto& global : program.globals) visitExpr(global->expr, operations);
            for (const auto& clause : program.clauses) {
                visitCall(clause->head, operations);
                for (const auto& annotation : clause->annotations) visitCall(annotation, operations);
                for (const auto& goal : clause->body) visitGoal(goal, operations);
                for (const auto& branch : clause->fallbackBranches) for (const auto& goal : branch) visitGoal(goal, operations);
            }
        ...
        if (records.empty()) throw std::runtime_error("no deterministically resolved mixfix expressions were found");
