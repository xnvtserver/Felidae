...
int Felidae::runMixfixGruTraining(int argc, char** argv) {
    ...
    try {
        ...
        std::ostringstream expectedSentencePieceHash;
        expectedSentencePieceHash << "fnv1a64:" << std::hex
        ...
        for (const auto& dataPath : dataPaths) {
            ...
            while (std::getline(dataset, line)) {
                ...
                if (!record.contains("sentencepiece_model_hash") ||
                    ...
                    record.at("sentencepiece_model_hash").get<std::string>() != expectedSentencePieceHash.str()) {
        ...
        std::ostringstream sentencePieceHash;
        sentencePieceHash << "fnv1a64:" << std::hex
                          << fnv1a(FELIDAE_SENTENCEPIECE_MODEL_PATH);
        auto verified = Felidae::GruMixfixStateModel::loadVersioned(
            config, outputDirectory, sentencePieceHash.str());
        (void)verified;
