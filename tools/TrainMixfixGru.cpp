...
int Felidae::runMixfixGruTraining(int argc, char** argv) {
    ...
    try {
        ...
        const std::string expectedSentencePieceHash(argv[8]);
        if (!expectedSentencePieceHash.starts_with("sha256:") ||
            expectedSentencePieceHash.size() != 71) {
            throw std::runtime_error("compiler training requires an exact SHA-256 SentencePiece identity");
        }
        struct Sample {
        ...
        for (const auto& dataPath : dataPaths) {
            ...
            while (std::getline(dataset, line)) {
                ...
                if (!record.contains("schema_version") || !record.at("schema_version").is_number_unsigned() ||
                    record.at("schema_version").get<std::uint64_t>() != 2) {
                    throw std::runtime_error("mixfix JSONL schema version is incompatible");
                }
                if (!record.contains("sentencepiece_model_hash") ||
                    !record.at("sentencepiece_model_hash").is_string() ||
                    record.at("sentencepiece_model_hash").get<std::string>() != expectedSentencePieceHash) {
                    throw std::runtime_error("mixfix JSONL was generated by a different SentencePiece model");
                }
                if (!record.contains("compiler_ir_vocabulary") ||
                    ...
                    throw std::runtime_error("mixfix JSONL compiler IR vocabulary is incompatible");
                }
                const auto input = ids(record, "input_ids");
                if (input.empty()) throw std::runtime_error("mixfix JSONL input_ids cannot be empty");
                ...
                if (record.contains("rejection_stage")) {
                    if (!record.contains("target_ids")) throw std::runtime_error("mixfix rejection record requires a REJECT teacher target");
                    ++rejectedRecords;
                }
                auto target = ids(record, "target_ids");
                if (target.empty()) throw std::runtime_error("mixfix JSONL target_ids cannot be empty");
                ...
                const auto vocabulary = Felidae::makeMixfixContext(Felidae::FelidaeIr{}).outputVocabulary;
                const auto firstKind = vocabulary.at(static_cast<std::size_t>(target.front())).kind;
                ...
                if (record.contains("rejection_stage")) {
                    if (declaredDecision != "REJECT" || target.size() != 1 ||
                        ...
                        throw std::runtime_error("mixfix rejection record target must be exactly REJECT");
                    }
                } else if (declaredDecision == "ABSTAIN") {
                    if (target.size() != 1 || firstKind != Felidae::MixfixIrTokenKind::Abstain) {
                        throw std::runtime_error("mixfix abstention record target must be exactly ABSTAIN");
                    }
                    ++abstainedRecords;
                } else {
                    if (!declaredDecision.empty() && declaredDecision != "ACCEPT") {
                        throw std::runtime_error("mixfix JSONL decision must be ACCEPT, REJECT, or ABSTAIN");
                    }
                    if (firstKind != Felidae::MixfixIrTokenKind::Accept) {
                        throw std::runtime_error("mixfix executable teacher target must begin with ACCEPT");
                    }
                }
                Sample sample;
        ...
        if (targetFamilies.size() < 3) throw std::runtime_error("mixfix dataset requires at least three structural families for train/validation/test splits");
        std::vector<std::vector<std::size_t>*> familyOrder;
        for (auto& [target, family] : targetFamilies) { (void)target; familyOrder.push_back(&family); }
        ...
        for (auto* family : familyOrder) {
            ...
            // Invalid and unresolved examples must actually teach REJECT and
            // ABSTAIN. Keep each complete decision family in training; only
            // executable structural families rotate through held-out splits.
            auto* destination = &training;
            if (decision == Felidae::MixfixIrTokenKind::Accept) {
                const auto partition = heldOutIndex++ % 10;
                if (partition == 0) destination = &test;
                else if (partition == 1) destination = &validation;
            }
            destination->insert(destination->end(),family->begin(),family->end());
        }
        if (training.empty()||validation.empty()||test.empty()) throw std::runtime_error("mixfix structural-family split produced an empty partition");
        ...
        for (std::size_t epoch = 0; epoch < epochs; ++epoch) {
            ...
            const auto epochSeconds = std::chrono::duration<double>(
                std::chrono::steady_clock::now() - epochStarted).count();
        ...
        double testLoss=0.0;std::size_t testExact=0,testInvalid=0;
        for(const auto index:test){const auto& sample=samples[index];testLoss+=model.evaluateTeacherForced(sample.input,sample.target);try{const auto decoded=model.transform(sample.input,evaluationContext);std::vector<Felidae::MixfixVocabularyId> expected;for(const auto value:sample.target)expected.push_back(static_cast<Felidae::MixfixVocabularyId>(value));if(decoded==expected)++testExact;}catch(const Felidae::IrError&){++testInvalid;}}
        std::cout<<"test_loss="<<(testLoss/test.size())<<" autoregressive_exact="<<testExact<<'/'<<test.size()<<" invalid_autoregressive="<<testInvalid<<'/'<<test.size()<<"\n";
        ...
        if (!manifest) throw std::runtime_error("cannot write compiler model manifest");
        manifest << "model_version=gru-v1\n"
                 << "backend=torchscript-gru\n"
                 << "sentencepiece_model_hash=" << expectedSentencePieceHash << "\n"
        ...
        manifest.close();
        if (!manifest) throw std::runtime_error("cannot finalize compiler model manifest");
        auto verified = Felidae::GruMixfixStateModel::loadVersioned(
            config, outputDirectory, expectedSentencePieceHash);
        (void)verified;
