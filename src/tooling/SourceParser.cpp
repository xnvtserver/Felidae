...
fs::path findProjectRoot(fs::path start) {
  ...
  while (!start.empty()) {
    if (fs::is_directory(start / "core", ec) &&
        fs::is_directory(start / "src", ec)) {
      return start;
    }
...
std::vector<fs::path> resolveImport(const fs::path &baseDir,
  ...
  if (wildcard) {
    ...
    if (relative.extension().empty())
      candidates.push_back(baseDir / (relative.string() + ".fx"));
    if (!projectRoot.empty()) {
      ...
      if (relative.extension().empty()) {
        ...
        candidates.push_back(projectRoot / "core" /
                             (relative.string() + ".fx"));
        candidates.push_back(projectRoot / "stdlib" /
                             (relative.string() + ".fx"));
      }
    }
    std::error_code ec;
...
std::vector<std::string> scanImports(const fs::path &file) {
  ...
  const Program program = parseProgramFile(file);
  for (const auto &statement : program.imports) {
    imports.insert(imports.end(), statement->paths.begin(),
                   statement->paths.end());
  }
  return imports;
}
...
SourceGraph collectSourceGraph(const fs::path &entry,
                               const fs::path &projectRoot, bool loadImports) {
  SourceGraph graph;
  ...
  const auto visit = [&](const auto &self, fs::path file) -> void {
    ...
    if (visited.count(file) > 0)
      return;
    if (!visiting.insert(file).second)
      return;
    if (loadImports) {
      for (const auto &importName : scanImports(file)) {
        auto resolved =
            resolveImport(file.parent_path(), projectRoot, importName);
        if (resolved.empty()) {
          graph.unresolvedImports.push_back(importName);
          continue;
        }
        for (const auto &dependency : resolved)
  ...
  graph.unresolvedImports.erase(std::unique(graph.unresolvedImports.begin(),
...
void parseFileWithRegistry(
    const fs::path &file, const std::shared_ptr<OperatorRegistry> &operators,
    const std::function<void(std::shared_ptr<Statement>)> &consume) {
  Program program =
      parseProgramText(readSourceFile(file), operators, CompilerOptions{});
...
void parseFileStatements(
  ...
  Program program = parseProgramFile(entry);
  for (auto &statement : program.statements)
    consume(std::move(statement));
}

fs::path resolveEntryPath(const fs::path &path) {
  const auto resolved = resolveProgramEntryPath(path);
  std::error_code ec;
...
LoadedProgram loadProgram(const fs::path &entryFile, bool loadImports) {
  ...
  for (const auto &file : graph.dependencyOrder) {
    parseFileWithRegistry(file, loaded.operators,
...
std::vector<std::string> listCoreLibraries(const fs::path &startDir) {
  ...
  for (fs::directory_iterator it(core, ec), end; !ec && it != end;
       it.increment(ec)) {
    if (it->is_regular_file(ec) && it->path().extension() == ".fx") {
