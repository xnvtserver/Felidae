...
static void printDebugUsage(std::ostream &out) {
  out << "Felidae AST debugger " << LANGUAGE_VERSION << "\n"
      ...
      << "  --check-json     Emit structured JSON diagnostics for editor "
         "integrations.\n"
      ...
      << "  --list-libraries Print a JSON array of importable core library "
         "names.\n"
      << "                   Resolved relative to <file.fx> when given, "
         "otherwise the\n"
      << "                   current directory. Editor integrations should "
         "call this\n"
      << "                   instead of hand-maintaining a copy of the module "
         "list.\n"
      << "  --list-builtins  Print a JSON array of {name, effect} builtin "
         "functions.\n"
      << "  --symbols-json   Print a JSON symbol table (methods, facts, "
         "globals with\n"
      << "                   source spans and declared params, plus resolved "
         "files and\n"
      << "                   unresolved imports) for <file.fx>. Params back "
         "editor\n"
      << "                   signature help. Always follows imports, like "
         "--lsp.\n"
      << "  --operators-json Print versioned, non-executing dynamic operator "
         "metadata\n"
      << "                   for <file.fx>, including imported public "
         "contracts.\n"
...
static void printAstDiagnostics(const std::vector<AstDiagnostic> &diagnostics) {
  for (const auto &diagnostic : diagnostics) {
    std::cout << "FELIDAE_DIAGNOSTIC "
              << "severity=" << diagnostic.severity << " code="
...
static const char *builtinEffectName(BuiltinEffect effect) {
  ...
  case BuiltinEffect::Pure:
    return "pure";
  case BuiltinEffect::ReadsExternalState:
    return "reads";
  case BuiltinEffect::WritesExternalState:
    return "writes";
  case BuiltinEffect::Volatile:
    return "volatile";
  }
  return "volatile";
}
...
static std::string builtinsJson(const std::vector<BuiltinInfo> &infos) {
  ...
  for (size_t i = 0; i < infos.size(); ++i) {
    if (i)
      out << ",";
    out << "{\"name\":\"" << jsonEscape(infos[i].name) << "\",\"effect\":\""
        << builtinEffectName(infos[i].effect) << "\"}";
  }
  out << "]";
  return out.str();
}
...
static std::string
symbolDefinitionsJson(const std::vector<SymbolDefinition> &definitions) {
  std::ostringstream out;
  out << "[";
...
static std::string
symbolsJson(const SymbolSummary &symbols, const std::vector<fs::path> &files,
...
static const char *operatorPrecedenceName(OperatorPrecedence value) {
  switch (value) {
  case OperatorPrecedence::Control:
    return "control";
  case OperatorPrecedence::LogicalOr:
    return "logical-or";
  case OperatorPrecedence::LogicalAnd:
    return "logical-and";
  case OperatorPrecedence::Relationship:
    return "relationship";
  case OperatorPrecedence::Ordering:
    return "ordering";
  case OperatorPrecedence::Pipeline:
    return "pipeline";
...
static const char *operatorCardinalityName(OperatorCardinality value) {
  switch (value) {
  case OperatorCardinality::One:
    return "one";
  case OperatorCardinality::Optional:
    return "optional";
  case OperatorCardinality::Many:
    return "many";
  }
  return "one";
}
...
static std::string
operatorBindingsJson(const std::vector<OperatorTypeBinding> &bindings) {
  std::ostringstream out;
  out << "[";
  for (size_t i = 0; i < bindings.size(); ++i) {
    ...
    out << "{\"name\":\"" << jsonEscape(bindings[i].name)
        << "\",\"nameId\":" << bindings[i].nameId << ",\"type\":\""
        << jsonEscape(bindings[i].type)
...
static std::string
operatorMetadataJson(const OperatorRegistry &registry,
                     const std::vector<fs::path> &files,
                     const std::vector<std::string> &unresolvedImports) {
  std::ostringstream out;
  out << "{\"protocol\":\"felidae.operator-metadata\",\"version\":1,"
         "\"patterns\":[";
  ...
  for (size_t i = 0; i < patterns.size(); ++i) {
    ...
    out << "{\"operatorId\":" << pattern.operatorId
        << ",\"patternId\":" << pattern.patternId << ",\"operator\":\""
        << jsonEscape(pattern.operatorName) << "\",\"pattern\":\""
        << jsonEscape(pattern.pattern) << "\",\"precedence\":\""
        << operatorPrecedenceName(pattern.precedence)
        << "\",\"associativity\":\""
        << operatorAssociativityName(pattern.associativity)
        << "\",\"fixity\":\"" << operatorFixityName(pattern.fixity)
        << "\",\"visibility\":\"" << operatorVisibilityName(pattern.visibility)
        << "\",\"module\":\"" << jsonEscape(pattern.module)
        << "\",\"anchors\":[";
  ...
  for (size_t i = 0; i < overloads.size(); ++i) {
    ...
    out << "{\"operatorId\":" << overload.operatorId
        << ",\"patternId\":" << overload.patternId << ",\"method\":\""
        << jsonEscape(overload.methodName)
        ...
        << "\",\"cardinality\":\""
        << operatorCardinalityName(overload.cardinality) << "\",\"effects\":\""
        << (overload.effect == OperatorEffect::Pure ? "pure" : "impure")
  ...
  for (size_t i = 0; i < matchers.size(); ++i) {
    ...
    out << "{\"operatorId\":" << matcher.operatorId
        << ",\"patternId\":" << matcher.patternId << ",\"method\":\""
        << jsonEscape(matcher.methodName)
...
static std::string versionJson(const char *toolName) {
  std::ostringstream out;
  out << "{\"name\":\"" << jsonEscape(toolName) << "\",\"version\":\""
...
static bool hasErrorDiagnostic(const std::vector<AstDiagnostic> &diagnostics) {
  return std::any_of(diagnostics.begin(), diagnostics.end(),
                     [](const AstDiagnostic &diagnostic) {
...
static std::string
diagnosticsJson(const std::vector<AstDiagnostic> &diagnostics) {
  std::ostringstream out;
  out << "{\"ok\":" << (hasErrorDiagnostic(diagnostics) ? "false" : "true")
      << ",\"diagnostics\":[";
  for (size_t i = 0; i < diagnostics.size(); ++i) {
    ...
    int endColumn =
        std::max(endLine == line ? column : 1, diagnostic.endColumn);
    out << "{\"severity\":\"" << jsonEscape(diagnostic.severity)
        << "\",\"lspSeverity\":" << lspSeverity(diagnostic.severity)
        << ",\"line\":" << line << ",\"column\":" << column
        << ",\"endLine\":" << endLine << ",\"endColumn\":" << endColumn
        << ",\"code\":\""
        << jsonEscape(diagnostic.code.empty() ? "felidae.ast" : diagnostic.code)
        << "\""
...
static std::string analyzeTextJson(const fs::path &path,
  ...
  try {
    ...
    if (cached != textCache.end() && cached->second.size == text.size() &&
        cached->second.hash == hash) {
...
static std::string symbolsFileJson(const fs::path &path, bool loadImports) {
  try {
    ...
    return symbolsJson(analysis.symbols(), loaded.files,
                       loaded.unresolvedImports);
  } catch (const std::exception &e) {
    return errorJson(e.what());
  }
}
...
static std::string operatorsFileJson(const fs::path &path, bool loadImports) {
  try {
    ...
    if (!loaded.operators)
      return errorJson("Operator metadata registry was not created");
      ...
      for (const auto &annotation : clause->annotations) {
        ...
        const auto *pattern =
            parsed.pattern.empty()
                ...
                : loaded.operators->findPattern(parsed.operatorName,
                                                parsed.pattern);
        if (!pattern)
        ...
        if (annotation.builtinId == BuiltinId::MatcherAnnotation) {
          loaded.operators->registerMatcher(makeOperatorMatcherDefinition(
              parsed, *pattern, clause->head.name, clause->head.nameId,
              clause->module));
        } else {
    ...
    return operatorMetadataJson(*loaded.operators, loaded.files,
                                loaded.unresolvedImports);
  } catch (const std::exception &e) {
    return errorJson(e.what());
  }
...
static std::string documentSymbolsJson(const SymbolSummary &symbols) {
  ...
  auto emit = [&](const std::vector<SymbolDefinition> &group, int kind) {
    for (const auto &definition : group) {
      ...
      first = false;
      out << "{\"name\":\"" << jsonEscape(definition.name)
          << "\",\"kind\":" << kind << ",\"detail\":\"" << jsonEscape(detail)
          << "\",\"range\":{\"start\":{\"line\":" << line
          << ",\"character\":" << column << "},\"end\":{\"line\":" << endLine
          << ",\"character\":" << endColumn
          << "}},\"selectionRange\":{\"start\":{\"line\":" << line
          << ",\"character\":" << column << "},\"end\":{\"line\":" << line
          << ",\"character\":" << column << "}}}";
    }
  };
...
static void publishDiagnostics(const std::string &uri,
                               const std::string &checkJson) {
  const auto analysis = nlohmann::json::parse(checkJson);
  auto diagnostics = nlohmann::json::array();
  for (const auto &diagnostic : analysis.at("diagnostics")) {
    const int line = std::max(0, diagnostic.value("line", 1) - 1);
    const int column = std::max(0, diagnostic.value("column", 1) - 1);
    const int endLine =
        std::max(line, diagnostic.value("endLine", line + 1) - 1);
    const int endColumn =
        std::max(endLine == line ? column + 1 : 0,
                 diagnostic.value("endColumn", column + 2) - 1);
    diagnostics.push_back(
        {{"range",
          {{"start", {{"line", line}, {"character", column}}},
           {"end", {{"line", endLine}, {"character", endColumn}}}}},
         {"severity", diagnostic.value("lspSeverity", 2)},
         {"source", "Felidae Debugger"},
         {"message", diagnostic.value("message", std::string{})}});
  }
  writeLspMessage(nlohmann::json{
      {"jsonrpc", "2.0"},
...
static bool readLspMessage(std::string &body) {
  ...
  while (std::getline(std::cin, line)) {
    ...
    if (line.rfind(header, 0) == 0) {
      contentLength =
          static_cast<size_t>(std::stoul(trimText(line.substr(header.size()))));
    }
  }
  if (contentLength == 0)
    return false;
...
static SymbolSummary
symbolsForDocument(const std::map<std::string, std::string> &documents,
...
static std::string wordAtPosition(const std::string &text, int line,
                                  int character) {
  size_t offset = 0;
  ...
  size_t start = cursor;
  size_t end = cursor;
  while (start > lineStart && isNameChar(text[start - 1]))
    --start;
  while (end < lineEnd && isNameChar(text[end]))
    ++end;
  if (start >= end)
    return {};
...
static std::string
definitionJson(const std::map<std::string, std::string> &documents,
  ...
  for (const auto *group :
       {&symbols.methods, &symbols.facts, &symbols.globals}) {
    for (const auto &definition : *group) {
      if (normalizeSymbolLookupName(definition.name) != normalized)
        continue;
      if (definition.spans.empty())
        continue;
      const SourceSpan &span = definition.spans.back();
...
static int runLspServer() {
  ...
  while (readLspMessage(body)) {
    ...
    const std::string method = methodValue->get<std::string>();
    const bool hasId = request.contains("id");
    const auto id = hasId ? request.at("id") : nlohmann::json(nullptr);
    ...
    const auto textDocument =
        params.value("textDocument", nlohmann::json::object());
    ...
    const auto uriValue = textDocument.find("uri");
    const std::string uri =
        uriValue != textDocument.end() && uriValue->is_string()
            ? uriValue->get<std::string>()
            : std::string{};
    if (method == "initialize") {
      ...
      writeLspMessage(
          nlohmann::json{{"jsonrpc", "2.0"},
                         {"id", id},
                         {"result", nlohmann::json::parse(documentSymbolsJson(
                                        symbolsForDocument(documents, uri)))}}
              .dump());
    } else if (method == "textDocument/definition") {
      const auto position = params.value("position", nlohmann::json::object());
      writeLspMessage(nlohmann::json{
          {"jsonrpc", "2.0"},
          {"id", id},
          {"result", nlohmann::json::parse(definitionJson(
                         documents, uri, std::max(0, position.value("line", 0)),
                         std::max(0, position.value("character", 0))))}}
                          .dump());
    } else if (method == "shutdown") {
      writeLspMessage(
          nlohmann::json{{"jsonrpc", "2.0"}, {"id", id}, {"result", nullptr}}
              .dump());
    } else if (method == "exit") {
      return 0;
      ...
      if (textDocument.contains("text")) {
        documents[uri] = textDocument.at("text").get<std::string>();
      } else if (const auto changes = params.find("contentChanges");
                 changes != params.end() && changes->is_array() &&
                 !changes->empty() && changes->back().contains("text")) {
        documents[uri] = changes->back().at("text").get<std::string>();
      } else if (params.contains("text")) {
        documents[uri] = params.at("text").get<std::string>();
      }
      auto existing = documents.find(uri);
      std::string result =
          existing != documents.end()
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    if (!dataOutputOnly) {
      std::cout << "FELIDAE_DEBUG_READY program="
    ...
    for (auto &diagnostic : astDiagnostics)
      diagnostic.file = analyzedFile;
    const auto analysisMicros =
