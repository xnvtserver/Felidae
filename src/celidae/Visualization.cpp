...
void visitGoal(const std::shared_ptr<Goal>& goal,
    ...
    if (auto call = std::dynamic_pointer_cast<CallGoal>(goal)) {
        ...
        visitGoal(assign->goal, visitCall);
...
std::string standaloneHtml(const std::string& json) {
    ...
    out << "<!doctype html>\n<html lang=\"en\">\n<head>\n"
        ...
        << "main{min-width:0;overflow:hidden;background:#071110}"
        ...
        << "svg{display:block;width:100%;height:100vh}line{stroke:#376760;stroke-width:1.2}"
        << "circle{stroke:#b8fff7;stroke-width:1.2}text{fill:#d9fffb;font-size:12px}"
        ...
        << "</style></head><body>\n"
        << "<aside><h1>Celidae Fact Graph</h1><div id=\"metrics\"></div>"
        ...
        << "const svg=document.getElementById('graph'),w=1200,h=760,cx=w/2,cy=h/2,total=Math.max(n.length,1),by=new Map(n.map(x=>[x.id,x]));svg.setAttribute('viewBox','0 0 '+w+' '+h);"
        << "n.forEach((x,i)=>{const a=i/total*Math.PI*2,r=220+(i%6)*42;x.x=cx+Math.cos(a)*r;x.y=cy+Math.sin(a)*r});"
        << "e.forEach(x=>{const a=by.get(x.from),b=by.get(x.to);if(!a||!b)return;const l=document.createElementNS('http://www.w3.org/2000/svg','line');for(const [k,v] of Object.entries({x1:a.x,y1:a.y,x2:b.x,y2:b.y}))l.setAttribute(k,v);svg.appendChild(l)});"
        << "const c={fact:'#18f0d7',field:'#f7c948',method:'#6ca8ff',global:'#f27d9d',library:'#9ca3af'};"
        << "n.forEach(x=>{const g=document.createElementNS('http://www.w3.org/2000/svg','g'),q=document.createElementNS('http://www.w3.org/2000/svg','circle'),t=document.createElementNS('http://www.w3.org/2000/svg','text');q.setAttribute('cx',x.x);q.setAttribute('cy',x.y);q.setAttribute('r',x.kind==='fact'?18:12);q.setAttribute('fill',c[x.kind]||'#9ca3af');t.setAttribute('x',x.x+18);t.setAttribute('y',x.y+4);t.textContent=x.detail?x.label+' - '+x.detail:x.label;g.append(q,t);svg.appendChild(g)});"
        << "</script></body></html>\n";
    return out.str();
}
