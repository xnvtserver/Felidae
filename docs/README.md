...
```text
source.fx -> deterministic lexer + word vocabulary -> IntegerParser -> Program AST -> Interpreter
```

Build artifacts are created only in `build/`. Use the repository
[`README.md`](../README.md) for build, direct interpretation, the word
vocabulary tokenizer, mixfix, fact-memory, and source-reload guidance.
