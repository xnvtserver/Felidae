...
The dataset is JSON Lines schema v6, with one operation-level record per line:
`operation_symbol`, ordered `input_kinds`, sorted `fact_types`, sorted
...
references `0..15`, booleans `0` or `1`, nil `0`, or Degree milli-values
`0`, `250`, `500`, `750`, and `1000`.

It trains three explicit operation teachers: `fx.identity` preserves a typed
input; `fx.context.has_facts` and `fx.context.has_hierarchy` return crisp
booleans from the fact-store snapshot available at `SsmProcess` inference.
Binaries containing `SemanticEval`/`SsmProcess` still require a separate
...
Training uses a fixed-seed grouped split by `(operation_symbol, target action)`.
Repeated records from one operation/action family cannot appear in both train
and validation; rare families remain training-only and the trainer prints
per-family held-out accuracy. This is a baseline measurement, not evidence
that the current small corpus generalizes to arbitrary fact reasoning.
